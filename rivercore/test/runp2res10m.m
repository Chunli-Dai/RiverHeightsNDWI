...
clear data
data.x=xout(idx);data.y=yout(idy);data.z=jump(idy,idx);
...
writeGeotiff(ofile,data.x,data.y,uint8(tz),1,255,projstr) %wrong, M is just shoreline, not water mask.
	else %load merged water mask
	  data=readGeotiff('watermask50.tif');
	end

%40m resolution -> river can be too narrow, so a 40 m resoltuion may break the rivers into sections.
xout=data.x;yout=data.y;jump=data.z;
resrc=10;%10m
data.x=xout(1):resrc:xout(end);data.y=yout(1):-resrc:yout(end);
tz = interp2(xout,yout,jump,data.x,data.y','*nearest',0);
res1=resrc;
