...
%%--------------------------attention ---------------------------------%%%
%%this block of code does not work well: by adding new attribute but not able to write back
%%and it is not necessary in my work, since I only want to keep the coords and spatial info after image processing
%%commented by Zeng, 
%%to use this block of code, please remove the "0" in the first line and let the condition start to work
if 0  % isfield(info,'map_info')
    line = info.map_info;
    line(line == '{' | line == '}') = [];
