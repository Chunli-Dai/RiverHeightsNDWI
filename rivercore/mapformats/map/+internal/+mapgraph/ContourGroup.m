...
if ~isempty(f) && ~isempty(ax)
    ...
    csListener = addlistener(ax.ColorSpace,'MarkedDirty',@setColormapsCallback);
    ...
    function setColormapsCallback(~,~)
        if isvalid(h)
            ...
            if ishghandle(g) && strcmp(get(g,'BeingDeleted'),'off')
                f  = ancestor(h.HGGroup,'figure');
                if ~isempty(f)
                    colorSpace = ax.ColorSpace;
                    if strcmp(colorSpace.ColormapMode,'auto')
                        cmap = get(f,'Colormap');
                    else
                        cmap = colorSpace.Colormap;
                    end
                    setContourGroupColormaps(h, cmap);
                end
            end
