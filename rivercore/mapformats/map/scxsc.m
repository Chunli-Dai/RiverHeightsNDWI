...
%   twice. All inputs and outputs are in degrees. Outputs are column
%   vectors, regardless of the shape of the inputs. The outputs contain NaN
%   for pairs of circles that do not intersect or that coincide (including
%   circles of zero radius that coincide).
%
%   [...] = SCXSC(..., angleUnit) uses the string angleUnit, which matches
%   either 'degrees' or 'radians', to specify the units of the latitude,
%   longitude, and radius arguments.
%
%   MAT = SCXSC(...) returns a single 2-by-N output, MAT = [LAT LON].
%
%   Example
%   -------
%   lat1 = -10;
...
function [ilat1, ilon1, ilat2, ilon2, coincident, nonIntersecting] ...
    ...
    [lat1, lon1, rho1] = fitWithinHemisphere(lat1, lon1, rho1);
    [lat2, lon2, rho2] = fitWithinHemisphere(lat2, lon2, rho2);
...
function [phi, lambda, rho] = fitWithinHemisphere(phi, lambda, rho)
% Ensure that the interior of the small circle fits within a hemisphere,
% replacing the center with its antipode when necessary. All inputs and
% outputs are in degrees.

    if all(rho > 180)
        % Reflect all the centers. This includes the scalar case.
        [phi, lambda, rho] = reflect(phi, lambda, rho);
    elseif any(rho > 180)
        % Reflect some of the centers, after expanding scalars.
        if isscalar(phi)
            phi = phi + zeros(size(rho));
        end
        
        if isscalar(lambda)
            lambda = lambda + zeros(size(rho));
        end
        
        q = (rho(:) > 180);
        [phi(q), lambda(q), rho(q)] = reflect(phi(q), lambda(q), rho(q));
    end
end
