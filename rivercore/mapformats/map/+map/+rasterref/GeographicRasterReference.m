...
% Copyright 2010-2016 The MathWorks, Inc.
    ...
    methods (Hidden)
        function Rblock = georefblock(R, rowLimits, colLimits)
            ...
            %   startcol = 41;
            %   endcol = 120;
            %   Rblock = georefblock(R,[startrow endrow],[startcol endcol])
            %   topoblock = topo(startrow:endrow,startcol:endcol);
            ...
            % geographic raster reference superclass.)
            
            % Size of sub-block raster
            % (Start to assign fields in a temporary scalar struct, S.)
            S.RasterSize = [diff(rowLimits) + 1, diff(colLimits) + 1];
