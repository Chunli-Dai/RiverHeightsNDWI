...
if isstruct(R)
    ...
    if isPlaid(R.LatMesh,R.LonMesh)
        ...
        % Make sure all elements of R.LonMesh fall within the limits.
        R.LonMesh = lonlim(1) + wrapTo360(R.LonMesh - lonlim(1));
...
% Assume that each row of lonmesh is the same as all the others, and that
% the numerical values increase from column to column.
lon = interp1(lonmesh(1,:), x, '*linear');
...
for k = 1:numel(C)
    ...
    if lines
        ...
        if (lonlim(2) - lonlim(1) == 360)
            ...
            [lon, lat] = snapToLimits(lon(:), lat(:), lonlim, tolSnap);
