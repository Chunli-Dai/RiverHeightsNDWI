...
if numDataArgs < 2 && ~strcmp(type,'any')
    ...
    % For better error messaging, validate latitude. If it passes, then
    % issue not enough inputs error.
...
function numDataArgs = verifyNumberOfDataArgs(inputs)
...
if ~stringPattern(paramPos)
    ...
    % paramPos. 
     argPos = paramPos+1;
     validateattributes(inputs{paramPos},{'char'},{'nonempty'},mfilename,'NAME',argPos);
end
