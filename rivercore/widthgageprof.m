function [co]=widthgageprof(odir,c,lateq,loneq)
%Copied from run2/gageheights_test.m
%Given height time series, all individual water masks, mannually flagged heihts.
%To get: height-width curve; 
co=[];
...
for j=1:length(ghf(:,1))
  ymd=num2str(ghf(j,1));
  str=deblank(['ls ',deblank(odir),'/watermask*',ymd,'*tif']);
...
set(gcf, 'PaperSize', [8 3]);
hold all;
plot(T6f(idb&~idWV01&id1),gagewidth(idb&~idWV01&id1),'ro','Markersize',8,'linewidth',3)
plot(T6f(idb&~idWV01&idlt1),gagewidth(idb&~idWV01&idlt1),'bo','Markersize',8,'linewidth',3)
plot(T6f(idb&~idWV01&id0),gagewidth(idb&~idWV01&id0),'go','Markersize',8,'linewidth',3)
box on
%axis([347.5 349.8 347.5 349.8]+0.5)
legend('Bad water classification','Partly bad','Good classification')
ylabel('ArcticDEM width (m)');xlabel('ArcticDEM height (m)')
...
gwid=gwid.data(:,2)*ft2m; % to meter %discharge_va(ft^3/s)   chan_width(ft)      gage_height_va(ft)

save test1.mat -v7.3
id=find((epochwid(2:end)-epochwid(1:end-1))==0);
epochwid(id)=[];gwid(id)=[]; %Delete repeat data
gwidi = interp1( epochwid , gwid , epoch,'linear') ;
...
hold all;
if 0
plot(gh2i(idb&~idWV01&id1)+Bias,gwidi(idb&~idWV01&id1),'r>','Markersize',8,'linewidth',3)
plot(gh2i(idb&~idWV01&idlt1)+Bias,gwidi(idb&~idWV01&idlt1),'b>','Markersize',8,'linewidth',3)
plot(gh2i(idb&~idWV01&id0)+Bias,gwidi(idb&~idWV01&id0),'g>','Markersize',8,'linewidth',3)
box on
%axis([347.5 349.8 347.5 349.8]+0.5)
...
hold on;plot(gwidh+Bias,gwid,'mo')


idp=find(idb&~idWV01);
for j=1:length(idp)
i=idp(j);
plot([gh2i(i)+Bias T6f(i)],[gwidi(i) gagewidth(i)],'k-')
end
legend('Bad water classification','Partly bad','Good classification','USGS','USGS field measurement')
saveas(gcf,'gagewidthflag2','fig')

figure;plot(epochwid, gwid,'>-',epoch,gwidi,'ro');
datetick('x','yyyy')
set(gcf,'Color','white')
set(gca,'FontSize', 12);
set(gcf, 'PaperPosition', [0 0 8 3]);
set(gcf, 'PaperSize', [8 3]);
ylabel('Width (m)');xlabel('Date')
legend('USGS channel width', 'Interpolated width')
saveas(gcf,'USGSchannelwidth','fig')
