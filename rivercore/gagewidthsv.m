function [co]=gagewidth(odir,c,lateq,loneq)
%Plot gage width-height curve.
co=[];
...
ifile=([deblank(odir),'/gagewidth.txt']);
fid = fopen(ifile);
n = linecount(fid);
fid = fopen(ifile);
...
for j=1:length(ghf(:,1))
  ...
  ymdi=ghf(j,1);
  id=(gwf(:,1)==ymdi);
  fprintf(['Gage ',ymd, ' width: ',num2str(gwf(id,2))])
  gagewidthi=mean(gwf(id,2));
