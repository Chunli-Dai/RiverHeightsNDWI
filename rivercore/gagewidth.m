...
for j=1:length(ghf(:,1))
  ...
  ymdi=ghf(j,1);
  id=(gwf(:,1)==ymdi);
  fprintf(['\n Gage ',ymd, ' width: ',num2str(gwf(id,2)')])
  gwfi=gwf(id,2);
  gagewidthi=nanmean(gwfi(gwfi>0));
...
epochwid(id)=[];gwidh(id)=[];gwid(id)=[]; %Delete repeat data
