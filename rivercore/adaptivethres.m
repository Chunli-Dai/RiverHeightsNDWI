...
%yb=hb.Values(:);hbBin=hb.BinWidth;% hb.BinWidth could be 'nonuniform'.
yb=hb.Values(:);hbBin=mean([edgesb(2:end)-edgesb(1:end-1)]);
ha=histogram(a,edgesa,'Normalization','pdf','Facecolor','red'); %,'Edgecolor','green'
ya=ha.Values(:);haBin=mean([edgesa(2:end)-edgesa(1:end-1)]);
sigma0hat=ones(2,1);
for k=[1,2]
    if k==1
        y=ya;% hb.Values;
        ksai0=[mean1 std1]';
        x=ha.BinEdges(1:end-1)+haBin/2;
        xa=x;
    elseif k==2
        y=yb;% hb.Values;
        ksai0=[mean2 std2]';
        x=hb.BinEdges(1:end-1)+hbBin/2;
        xb=x;
    end
    lenf=length(y);m=2;
count=0; dksai=1;
...
count=count+1;
if (count>99) 
    badflag=1;ithreshold =[threshold threshold];sigma0hat=[1 1];
