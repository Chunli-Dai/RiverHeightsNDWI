...
%cls = cl; %use this instead of above, Chunli September 2019. -Results do not change much.
...
% centroid_threshold = len/numel(bkpts)/1.5; %use the following by Chunli
if numel(spacing) == 1
    centroid_threshold = len/numel(bkpts)/1.5;
else
    spacing2=nanmean(diff(cumdists(bkpts))); %spacing of each output node along centerline
    centroid_threshold = spacing2/1.5;
end
