function [Est,Data,xobs]=ProcessData(c,s,reAttach,p,Href,SiteName,Method,lateq,loneq)

%% process centerline
% z1 = utmzone([64.78 -148]); %6W
z1 = utmzone([lateq loneq]); %6W
...
[cutm(i).X,cutm(i).Y] =polarstereo_fwd(c(i).Y,c(i).X,[],[],70,-45);
end
...
% % chunli, calculate xobs
% [x1,y1]=mfwdtran(utmstruct,lateq,loneq);
[x1,y1]=polarstereo_fwd(lateq,loneq,[],[],70,-45);
dist=sqrt( (y1-cutm.Y).^2 + (x1 - cutm.X).^2);
[distmin,k]=min(dist);    
xobs=FDkm(k);
if distmin>1000 %meter
warning('Gage location is more than 1km far away from the centerline.')
end
...
%     [Data{i}.X,Data{i}.Y]=mfwdtran(utmstruct,Data{i}.lat,Data{i}.lon);
    [Data{i}.X,Data{i}.Y]=polarstereo_fwd(Data{i}.lat,Data{i}.lon,[],[],70,-45);
